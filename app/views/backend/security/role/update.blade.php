        ...
        <div class="row">
            <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <div class="jarviswidget" id="wid-id-1" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-custombutton="false" data-widget-collapsed="false" data-widget-sortable="false">
                    <header>
                        <span class="widget-icon"> <i class="fa fa-key"></i> </span>
                        <h2>{{ Lang::get('roles.h2_map_server_to_roles') }}</h2>
                    </header>
                    <div>
                        <div class="jarviswidget-editbox">
                        </div>
                        <div class="widget-body no-padding">
                            <form action="/backend/security/role/update_admin/{{ $server->id }}" method="post" id="user-form" novalidate="novalidate">
                                <div style="padding: 15px;">
                                    <select multiple="multiple" name="roles[]" id="initializeDuallistbox">
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id; }}"@if (in_array($role->id, $server_roles)) selected="true" @endif>{{ $user->username; }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="smart-form">
